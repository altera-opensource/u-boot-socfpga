...
{
	if (status)
		writel(REGULAR_BOOT_MAGIC, &sysmgr_regs->isw_handoff[7]);
	else
		writel(0, &sysmgr_regs->isw_handoff[7]);
}
...
bool is_regular_boot_valid(void)
{
	...
	status = readl(&sysmgr_regs->isw_handoff[7]);
...
int qspi_flash_software_reset(void)
{
	...
	if (!memcmp(flash->name, "N25", 3))
		writel(QSPI_N25_SOFT_RESET_COMMAND,
			&sysmgr_regs->romcode_qspiresetcommand);
