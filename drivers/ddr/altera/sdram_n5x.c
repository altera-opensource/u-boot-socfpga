		    ...
		    bool *need_calibrate, bool is_ddr_hang_be4_rst)
{
	...
	if (*need_calibrate) {
		...
		for (i = 0; i < ddr_handoff_info->phy_handoff_length;
		     i = i + 2) {
			/*
			...
			debug("%s: Abs addr: 0x%08llx, APB offset: 0x%08x ",
			      __func__, value + ddr_handoff_info->phy_base,
			      value);
...
int populate_ddr_handoff(struct ddr_handoff *handoff)
{
	...
	if (readl(next_section_header) == SOC64_HANDOFF_DDR_UMCTL2_MAGIC) {
		 ...
		 * section
		 */
		handoff->cntlr2_handoff_length = socfpga_get_handoff_size((void *)
				    ...
				    u32 mask, u32 msb_mask, u32 shift)
{
	...
	reg = readl((uintptr_t)reg_addr);
	debug("umclt2 register 0x%x value is 0%x after ", reg_addr, reg);
	debug("updating with train result\n");
}
