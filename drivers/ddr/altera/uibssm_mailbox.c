	       ...
	       u32 cmd_param_0, struct uib_mb_resp *resp)
{
	...
	cmd_req = (usr_cmd_opcode << 0) | (usr_cmd_type << 16);
	...
	debug("%s: STATUS_CMD_RESPONSE_ERROR: 0x%x\n", __func__,
	      (resp->cmd_resp_status >> 5) & 0x7);
	debug("%s: STATUS_GENERAL_ERROR: 0x%x\n", __func__,
	      (resp->cmd_resp_status >> 1) & 0xF);
