...
#define usb_base			CONFIG_SYS_USB_BASE
...
{
	int loop = 0;
	u32 gintmsk = __raw_readl(usb_base + DWC_GINTMSK);
	gintmsk |= DWC_GINTMSK_PRTINTMSK;
	__raw_writel(gintmsk, usb_base + DWC_GINTMSK);

	u32 hcfg = __raw_readl(usb_base + DWC_HCFG);
	hcfg &= ~DWC_HPRT_FSLSUPP;
	__raw_writel(hcfg, usb_base + DWC_HCFG);

	__raw_writel(DWC_GAHBCFG_GLBLINTRMSK, usb_base + DWC_GAHBCFG);

	u32 hprt = __raw_readl(usb_base + DWC_HPRT);
	hprt |= DWC_HPRT_PRTPWR;
	__raw_writel(hprt, usb_base + DWC_HPRT);
...
{
	u32 gahbcfg = __raw_readl(usb_base + DWC_GAHBCFG);
	if (init.dma)
	...
	__raw_writel(gahbcfg, usb_base + DWC_GAHBCFG);
	 ...
	 */
	if (!init.dma) {
		__raw_writel(__raw_readl(usb_base + DWC_GAHBCFG)
			| DWC_GAHBCFG_NPTXFEMPLVL, usb_base+DWC_GAHBCFG);
		__raw_writel(__raw_readl(usb_base + DWC_GAHBCFG)
			| DWC_GAHBCFG_PTXFEMPLVL, usb_base+DWC_GAHBCFG);
	}
...
static int dwc_read_packet(struct packet p, void *odata)
{
	...
	while (!(__raw_readl(usb_base + DWC_HCINT(p.channel))
		& DWC_HCINT_XFERCOMP))
		udelay(100);
	/* clear the xfer complete interrupt */
	__raw_writel(DWC_HCINT_XFERCOMP, usb_base + DWC_HCINT(p.channel));
...
static int dwc_send_packet(struct packet p, u32 *data, int datasize)
{
	...
	__raw_writel(__raw_readl(usb_base + DWC_HCINT(p.channel)) | (1<<0),
		usb_base + DWC_HCINT(p.channel));
	...
	if (dir_out) {
		...
		while (total_read < transfer_len) {
			...
			__raw_writel(__raw_readl(usb_base +
				DWC_HCCHARN(bulk_msg.channel))
				| DWC_HCCHARN_CHDIS | DWC_HCCHARN_CHENA,
				usb_base + DWC_HCCHARN(bulk_msg.channel));
...
static int ctrlreq_out_data_phase(struct usb_device *dev, unsigned long pipe,
	...
	while ((__raw_readl(usb_base + DWC_HCINT(control_msg.channel))
		& (DWC_HCINT_XFERCOMP)) != 0) {
		udelay(100);
	}
	return result;
}
...
static int ctrlreq_in_status_phase(struct usb_device *dev, unsigned long pipe)
{
	...
	__raw_writel(__raw_readl(usb_base + DWC_HCINT(control_msg.channel))
		| (DWC_HCINT_XFERCOMP),
		usb_base + DWC_HCINT(control_msg.channel));
...
static int ctrlreq_out_status_phase(struct usb_device *dev, unsigned long pipe)
{
	...
	control_msg.mps = MAX_PACKET_SIZE;
	control_msg.pid = DATA1;
	__raw_readl(usb_base + DWC_HCINT(control_msg.channel));
